...
FastAPI-сервис находится в `api/`. Перед финальной сборкой checkpoint следует
положить в `artifacts/model`; без него контейнер запускается в техническом
режиме с пустыми предсказаниями. Собрать и запустить отдельный контейнер API:
