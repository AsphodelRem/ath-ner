...
Полный цикл установки зависимостей, проверки данных, обучения, предикта,
оценки и упаковки артефактов находится в
[`notebooks/kaggle_train_predict_evaluate.ipynb`](notebooks/kaggle_train_predict_evaluate.ipynb).

Перед запуском Notebook нужно включить `GPU` и `Internet` в настройках Kaggle.
Основные параметры эксперимента собраны в одной конфигурационной ячейке. Режим
`full` использует весь датасет; режимы `medium` и `smoke` предназначены для
более коротких проверочных запусков.
