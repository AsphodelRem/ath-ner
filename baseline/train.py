...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--eval-attn-implementation", choices=("eager", "sdpa"))
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Ignore labels of entities that are cut by a sliding-window edge.",
    )
...
def evaluate_loss(
    ...
    for batch_index, batch in enumerate(
        tqdm(loader, desc="Dev loss", unit="batch", leave=False),
        start=1,
    ):
        ...
        if not math.isfinite(loss_value):
            raise RuntimeError(f"non-finite dev loss at batch {batch_index}")
...
def train_epoch(
    ...
    *,
    ...
    for batch_index, batch in enumerate(progress, start=1):
        ...
        if device.type == "mps" and batch_index % 250 == 0:
...
def run(args: argparse.Namespace) -> Path:
    ...
    tags = tags_for_scheme(args.tag_scheme)
    ...
    train_dataset = TokenizedNerDataset(
        ...
        mask_partial_entities=args.mask_partial_window_entities,
    ...
    # Fixed tensor shapes avoid an MPSGraph compilation/cache blow-up caused by
    # producing a different padded sequence length for nearly every batch.
    ...
    baseline_config = {
        ...
        "inference_attn_implementation": (
            args.eval_attn_implementation or args.attn_implementation
        ),
        "mask_partial_window_entities": args.mask_partial_window_entities,
    }
    for epoch in range(1, args.epochs + 1):
        ...
        training_attn_implementation = model.config._attn_implementation
        if args.eval_attn_implementation is not None:
            model.set_attn_implementation(args.eval_attn_implementation)
        dev_loss = evaluate_loss(model, dev_loader, device)
        if args.eval_attn_implementation is not None:
            model.set_attn_implementation(training_attn_implementation)
